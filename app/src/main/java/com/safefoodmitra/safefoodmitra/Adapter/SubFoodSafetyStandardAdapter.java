...
public class SubFoodSafetyStandardAdapter extends RecyclerView.Adapter<SubFoodSafetyStandardAdapter.Myview> {
    ...
    int row_index= -1;
    public SubFoodSafetyStandardAdapter(Activity context, List<SubFoodSafetyModel> subFoodSafetyModels) {
    ...
    public Myview onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        SubfoodsafetyitemBinding subfoodsafetyitemBinding= DataBindingUtil.inflate(layoutInflater, R.layout.subfoodsafetyitem,parent,false);
        return new Myview(subfoodsafetyitemBinding);
    }

    @Override
    public void onBindViewHolder(@NonNull final Myview holder, final int position) {
        SubFoodSafetyModel subFoodSafetyModel= subFoodSafetyModels.get(position);
        holder.subfoodsafetyitemBinding.fsmstitles.setText(subFoodSafetyModel.getCat_name());
        Utlity.Set_imageGlide(context,subFoodSafetyModels.get(position).getIcon_path(),holder.subfoodsafetyitemBinding.foodicon);
                ...
                if (subFoodSafetyModel.getId()!=null){
                    if (subFoodSafetyModel.getIs_subcats().equals("1") && subFoodSafetyModel.getIs_docs().equals("0")){
                        if (context instanceof SubfoodsafetystandardActivity) {
                            ((SubfoodsafetystandardActivity)context).subCatedata(subFoodSafetyModel.getId());
                        }
                    }

                    else if (subFoodSafetyModel.getIs_docs().equals("1") && subFoodSafetyModel.getIs_subcats().equals("0")){
                        if (context instanceof SubfoodsafetystandardActivity) {
                            ((SubfoodsafetystandardActivity)context).subcatepdffile2(subFoodSafetyModel.getId());
                        }
                    }
                        ...
                        if (context instanceof SubfoodsafetystandardActivity) {
                            ((SubfoodsafetystandardActivity)context).subCatedataduble(subFoodSafetyModel.getId());
                        }
                       /* if (context instanceof SubfoodsafetystandardActivity) {
                        ...
                        // Utlity.show_toast(context,"show");
                    }
        ...
        if(row_index==position){
            holder.subfoodsafetyitemBinding.foodicon.setBorderColor(context.getResources().getColor(R.color.purple_200));
            holder.subfoodsafetyitemBinding.fsmstitles.setTextColor(context.getResources().getColor(R.color.purple_200));
        }
        else
        {
            holder.subfoodsafetyitemBinding.fsmstitles.setTextColor(Color.BLACK);
            holder.subfoodsafetyitemBinding.foodicon.setBorderColor(context.getResources().getColor(R.color.white));
    ...
    public class Myview extends RecyclerView.ViewHolder {
        SubfoodsafetyitemBinding subfoodsafetyitemBinding;
        public Myview(SubfoodsafetyitemBinding subfoodsafetyitemBinding) {
            super(subfoodsafetyitemBinding.getRoot());
            this.subfoodsafetyitemBinding=subfoodsafetyitemBinding;
        }
    }
}