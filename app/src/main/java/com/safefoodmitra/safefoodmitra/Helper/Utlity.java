package com.safefoodmitra.safefoodmitra.Helper;
...
import com.safefoodmitra.safefoodmitra.Modals.Keymodal;
...
public class Utlity {
    ...
    private static  final String verisonname = "";
    private static  final String verisoncode = "";
    public static String Uerrolid = "";
   ...
   private static final int SECOND_MILLIS = 1000;
    ...
    public static void Set_image(String url, ImageView img) {
        ...
        Picasso.get().load(Apis.imageBaseicon_Url+url).error(R.drawable.ic_image_gallery).placeholder(R.drawable.ic_image_gallery).into(img);
    }

    public static void Set_imageGlide(Context context,String url, ImageView img) {
        if (!TextUtils.isEmpty(url));
    ...
    public static void Set_imageGlideCmfDetils(Context context,String url, ImageView img) {
        if (!TextUtils.isEmpty(url));
        Glide.with(context).load(Apis.imagecmfdetils+url).error(R.drawable.blankimage).placeholder(R.drawable.blankimage).into(img);
    }
    public static void Set_imageGlideSlide(Context context,String url, ImageView img) {
        if (!TextUtils.isEmpty(url));
        Glide.with(context).load(Apis.imageslides+url).error(R.drawable.safelogo).placeholder(R.drawable.safelogo).into(img);
    }

    public static void Set_image1(String url, ImageView img) {
        if (!TextUtils.isEmpty(url));
   ...
   public static String getDays(String dayes){
       try
       {
           SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy");
           Date past = format.parse(dayes);
           Date now = new Date();
    ...
    public static Keymodal get_user(Context activity) {
        local_db = activity.getSharedPreferences("user_db", Context.MODE_PRIVATE);
        if(!TextUtils.isEmpty(local_db.getString("user_db","")))
            return gson.fromJson(local_db.getString("user_db",""), Keymodal.class);
        else
            return new Keymodal();
    }
    ...
    */
    ...
    /*public static void startPayment(Activity activity, String amount, String desc) {
        Checkout checkout = new Checkout();
        checkout.setKeyID("rzp_test_j01tHcNDlp2bNC");
        ...
        try {
            ...
            options.put("name", Utlity.get_user(activity).getName());
            options.put("description", desc);
            //You can omit the image option to fetch the image from dashboard
            // options.put("image", "http://mdmobile.in/mdmobile15772596502097373948.jpg");
            options.put("currency", "INR");
            options.put("amount", String.valueOf((Integer.parseInt(amount)*100)));

            JSONObject preFill = new JSONObject();
            preFill.put("email", Utlity.get_user(activity).getEmail());
            preFill.put("contact", Utlity.get_user(activity).getMobile_no());
   ...
   */
    ...
    public static void show_date_picker_audit(Activity activity, final TextView tv) {
        final Calendar calendar = Calendar.getInstance();
        DatePickerDialog dialog = new DatePickerDialog(activity, new DatePickerDialog.OnDateSetListener() {
            @Override
            public void onDateSet(DatePicker arg0, int year, int month, int day_of_month) {
    ...
    public static void show_time_picker(Activity activity, final TextView tv){
        // Get Current Time
    ...
    public static boolean validation(Activity activity, ArrayList<Validation_custome> fileds) {
        ...
        for(Validation_custome validation_custome: fileds)
        {
            if(validation_custome.getType().equalsIgnoreCase("text")&& TextUtils.isEmpty(validation_custome.getEditText().getText().toString()))
            {
               ...
               // showSnackbar(validation_custome.getEditText().getHint()+" "+getResources().getString(R.string.reguired));
                is_valid=false;
                break;
            }
    ...
    public static void Logout(final Context context) {
            ...
            alertDialogBuilder.setMessage(R.string.surepopup);
            alertDialogBuilder.setPositiveButton(context.getString(R.string.yespopup),
            ...
            alertDialogBuilder.setNegativeButton(context.getString(R.string.nopopup), new DialogInterface.OnClickListener() {
                @Override
